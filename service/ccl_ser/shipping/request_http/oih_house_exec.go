...
func OihHouseExec2(login response_model.LoginResp, charge response_model.ShiGetOihChargesRows) (response_model.OihHouseExec, error) {
	var (
		client       = &http.Client{}
		request      = &http.Request{}
		err          error
		post         = &http.Response{}
		response     = response_model.OihHouseExec{}
	...
	_ = writer.WriteField("callurl", "/Oih/OihHouseExec")
	_ = writer.WriteField("ask", "ok")
	_ = writer.WriteField("id", fmt.Sprintf(`%d`, charge.PlmHblId))
	_ = writer.WriteField("formid", "46000")
	_ = writer.WriteField("op", "exec")
	_ = writer.WriteField("mode", "Exec")
	_ = writer.WriteField("cmd", "btn0InAPLineInvoiceOcean")
	_ = writer.WriteField("precallid", fmt.Sprintf(`%d`, charge.PlmMasterId))
	_ = writer.WriteField("precaller", "OimOimMaster")
	_ = writer.WriteField("caller", "OihOihHouse")
	_ = writer.WriteField("callf", "btn_btn0InAPLineInvoiceOcean")
	_ = writer.WriteField("calldoccode", docCodeHouse)
	_ = writer.WriteField("firstgroup", "")
	_ = writer.WriteField("objstr", fmt.Sprintf(`{"ids":[%v],"mid":%v,"group":"oih","Currency":false}`, charge.KeyId, charge.PlmHblId))
	_ = writer.WriteField("subform", "f_plm4_hbl_list_charge_inbound")
	err = writer.Close()
	if err != nil {
		return response, errors.Wrap(err, "请求OihHouseExec接口，关闭writer出现问题")
	}
	request, err = http.NewRequest("POST", "https://us.eftcloud.com/Oih/OihHouseExec", payload)
	if err != nil {
		return response, errors.Wrap(err, "请求OihHouseExec接口，创建HTTP连接失败")
	}
	...
	if err != nil {
		return response, errors.Wrap(err, "请求OihHouseExec接口，请求查询数据失败")
	}
	if post == nil || post.Body == nil {
		return response, errors.Wrap(err, "请求OihHouseExec接口，请求查询数据返回失败")
	}
	defer post.Body.Close()
	data, _ := io.ReadAll(post.Body)
	err = json.Unmarshal(data, &response)
	if err != nil {
		return response, errors.Wrap(err, "请求OihHouseExec接口，请求解析数据失败")
	}
	return response, nil
}

func OihHouseExec(login response_model.LoginResp, charge response_model.ShiGetOihChargesRows) (response_model.OihHouseExec, error) {
	var (
		client       = &http.Client{}
		request      = &http.Request{}
		err          error
