...
func Shipping(order model.OrderParam, param request_model.CCLParam) (int64, error) {
	...
	tools.Logger(param.Serial, "查询OI Master列表完成", fmt.Sprintf(`查询到的数据信息：%+v`, master.Rows[0]), "")
	...
	tools.Logger(param.Serial, "查询”List HBLs”列表完成", fmt.Sprintf(`查询到的数据信息：%+v`, hbls.Rows[0]), "")
	//-------------------请求GetOihCharges数据-------------------
