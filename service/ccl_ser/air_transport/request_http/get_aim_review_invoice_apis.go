...
func GetAimReviewInvoiceApis(rows response_model.AirQueryAimMasterRows, login response_model.LoginResp) (response_model.AirGetAimReviewInvoiceApis, error) {
	var (
		client   = &http.Client{}
		request  = &http.Request{}
		err      error
		post     = &http.Response{}
		response = response_model.AirGetAimReviewInvoiceApis{}
	...
	_ = writer.WriteField("filterStr", fmt.Sprintf(`[{"type":"string","field":"doc_code_master","value":"%s"},{"type":"numeric","field":"efc_company_id","value":"%d"}]`, rows.AimDocCode, rows.EfcCompanyId))
	...
	err = writer.Close()
	if err != nil {
		return response, errors.Wrap(err, "请求GetAimReviewInvoiceApis接口，关闭writer出现问题")
	}
	request, err = http.NewRequest("POST", "https://us.eftcloud.com/Aim/GetAimReviewInvoiceApis", payload)
	if err != nil {
		return response, errors.Wrap(err, "请求GetAimReviewInvoiceApis接口，创建HTTP连接失败")
	}
	...
	request.Header.Set("referer", "https://us.eftcloud.com/Aim/AimMaster?v=v3.1116-09&l=en-US&id=-99&mode=View")
	...
	if err != nil {
		return response, errors.Wrap(err, "请求GetAimReviewInvoiceApis接口，查询数据失败")
	}
	if post == nil || post.Body == nil {
		return response, errors.Wrap(err, "请求GetAimReviewInvoiceApis接口，数据返回失败")
	}
	defer post.Body.Close()
	data, _ := ioutil.ReadAll(post.Body)
	err = json.Unmarshal(data, &response)
	if err != nil {
		return response, errors.Wrap(err, "请求GetAimReviewInvoiceApis接口，解析数据失败")
	}
	return response, err
}
