...
func GetAimHbls(aimMasterId int64, login response_model.LoginResp) (response_model.AirGetAimHbls, error) {
	var (
		client   = &http.Client{}
		request  = &http.Request{}
		err      error
		post     = &http.Response{}
		response = response_model.AirGetAimHbls{}
		payload  = &bytes.Buffer{}
		writer   = multipart.NewWriter(payload)
	)
	_ = writer.WriteField("caller", "")
	_ = writer.WriteField("callid", fmt.Sprintf(`%d`, aimMasterId)) // aim_master_id
	_ = writer.WriteField("sortName", "aih_hbl_display_seq")
	_ = writer.WriteField("filterStr", fmt.Sprintf(`[{"type":"numeric","field":"aim_master_id","value":"%d"}]`, aimMasterId))
	...
	_ = writer.WriteField("sort", "aih_hbl_id")
	_ = writer.WriteField("order", "asc")
	err = writer.Close()
	if err != nil {
		return response, errors.Wrap(err, "请求GetAimHbls接口，关闭writer出现问题")
	}
	request, err = http.NewRequest("POST", "https://us.eftcloud.com/Aim/GetAimHbls", payload)
	if err != nil {
		return response, errors.Wrap(err, "请求GetAimHbls接口，创建HTTP连接失败")
	}
	...
	if err != nil {
		return response, errors.Wrap(err, "请求GetAimHbls接口，请求查询数据失败")
	}
	if post == nil || post.Body == nil {
		return response, errors.Wrap(err, "请求GetAimHbls接口，请求查询数据返回失败")
	}
	defer post.Body.Close()
	data, _ := io.ReadAll(post.Body)
	err = json.Unmarshal(data, &response)
	if err != nil {
		return response, errors.Wrap(err, "请求GetAimHbls接口，请求解析数据失败")
	}
	return response, nil
}
