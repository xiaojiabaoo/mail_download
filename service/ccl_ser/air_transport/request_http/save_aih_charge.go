...
func SaveAihChargeJsonParam(charges response_model.AirGetAihChargesRows) request_model.SaveAihChargeParam {
	var (
		param = request_model.SaveAihChargeParam{}
	)
...
func SaveAihCharge(login response_model.LoginResp, charges response_model.AirGetAihChargesRows) (response_model.AirSaveAihCharge, error) {
	var (
		client   = &http.Client{}
		request  = &http.Request{}
		err      error
		post     = &http.Response{}
		response = response_model.AirSaveAihCharge{}
		param    = request_model.SaveAihChargeParam{}
		payload  = &bytes.Buffer{}
		writer   = multipart.NewWriter(payload)
		marshal  []byte
	)
	param = SaveAihChargeJsonParam(charges)
	marshal, err = json.Marshal(param)
	if err != nil {
		return response, errors.Wrap(err, "序列化SaveAihCharge参数失败")
	}
	...
	err = writer.Close()
	if err != nil {
		return response, errors.Wrap(err, "请求SaveAihCharge接口，关闭writer出现问题")
	}
	request, err = http.NewRequest("POST", "https://us.eftcloud.com/Aih/SaveAihCharge", payload)
	if err != nil {
		return response, errors.Wrap(err, "请求SaveAihCharge接口，创建HTTP连接失败")
	}
	...
	if err != nil {
		return response, errors.Wrap(err, "请求SaveAihCharge接口，请求查询数据失败")
	}
	if post == nil || post.Body == nil {
		return response, errors.Wrap(err, "请求SaveAihCharge接口，请求查询数据返回失败")
	}
	defer post.Body.Close()
	data, _ := io.ReadAll(post.Body)
	err = json.Unmarshal(data, &response)
	if err != nil {
		return response, errors.Wrap(err, "请求SaveAihCharge接口，请求解析数据失败")
	}
	return response, nil
}
