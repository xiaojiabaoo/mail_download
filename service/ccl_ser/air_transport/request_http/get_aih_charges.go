...
func GetAihCharges(aihHblId int64, login response_model.LoginResp) (response_model.AirGetAihCharges, error) {
	var (
		client   = &http.Client{}
		request  = &http.Request{}
		err      error
		post     = &http.Response{}
		response = response_model.AirGetAihCharges{}
	...
	_ = writer.WriteField("sort", "plm_charge_cost_id")
	_ = writer.WriteField("order", "asc")
	err = writer.Close()
	if err != nil {
		return response, errors.Wrap(err, "请求GetAihCharges接口，关闭writer出现问题")
	}
	request, err = http.NewRequest("POST", "https://us.eftcloud.com/Aih/GetAihCharges", payload)
	if err != nil {
		return response, errors.Wrap(err, "请求GetAihCharges接口，创建HTTP连接失败")
	}
	...
	if err != nil {
		return response, errors.Wrap(err, "请求GetAihCharges接口，请求查询数据失败")
	}
	if post == nil || post.Body == nil {
		return response, errors.Wrap(err, "请求GetAihCharges接口，请求查询数据返回失败")
	}
	defer post.Body.Close()
	data, _ := io.ReadAll(post.Body)
	err = json.Unmarshal(data, &response)
	if err != nil {
		return response, errors.Wrap(err, "请求GetAihCharges接口，请求解析数据失败")
	}
	return response, nil
}
