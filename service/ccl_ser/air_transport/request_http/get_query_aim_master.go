...
func GetQueryAimMaster(info response_model.AirQueryInfo, masterMawbNo string, login response_model.LoginResp) (response_model.AirQueryAimMaster, error) {
	var (
		client   = &http.Client{}
		request  = &http.Request{}
		err      error
		post     = &http.Response{}
		response = response_model.AirQueryAimMaster{}
	...
	err = writer.Close()
	if err != nil {
		return response, errors.Wrap(err, "请求GetQueryAimMaster接口，关闭writer出现问题")
	}
	request, err = http.NewRequest("POST", "https://us.eftcloud.com/Aim/GetQueryAimMaster", payload)
	if err != nil {
		return response, errors.Wrap(err, "请求GetQueryAimMaster接口，创建HTTP连接失败")
	}
	...
	request.Header.Set("referer", fmt.Sprintf("https://us.eftcloud.com/Aim/QueryAimMaster?vid=%v&qv=%d&l=en-US&op=get&mode=Query&callid=0", vid, info.Ver))
	...
	if err != nil {
		return response, errors.Wrap(err, "请求GetQueryAimMaster接口，查询数据失败")
	}
	if post == nil || post.Body == nil {
		return response, errors.Wrap(err, "请求GetQueryAimMaster接口，数据返回失败")
	}
	defer post.Body.Close()
	data, _ := ioutil.ReadAll(post.Body)
	err = json.Unmarshal(data, &response)
	if err != nil {
		return response, errors.Wrap(err, "请求GetQueryAimMaster接口，解析数据失败")
	}
	return response, err
}
