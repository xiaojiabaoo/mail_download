...
type AirGetAihCharges struct {
	Type    any                      `json:"type"` // 类型string，因为对应的Java语言String的默认值是null，以免报错
	Rows    []AirGetAihChargesRows   `json:"rows"`
	Footer  []AirGetAihChargesFooter `json:"footer"`
	Total   int64                    `json:"total"`
	Success bool                     `json:"success"`
	Err     bool                     `json:"err"`
}

type AirGetAihChargesRows struct {
...
type AirGetAihChargesFooter struct {
	TableKeyName          any     `json:"TableKeyName"` // 类型string，因为对应的Java语言String的默认值是null，以免报错
